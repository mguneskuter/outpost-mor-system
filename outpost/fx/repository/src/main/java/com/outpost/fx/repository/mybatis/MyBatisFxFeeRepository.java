...
public final class MyBatisFxFeeRepository implements FxFeeRepository {
   ...
   */
  @Override
  public List<com.outpost.fx.FxFee> findFxFees() {
    return Objects.requireNonNull(mapper.findFxFees(), "FX fee rows").stream()
        .map(row -> toFxFee(Objects.requireNonNull(row, "FX fee row")))
        .toList();
  }

  private static com.outpost.fx.FxFee toFxFee(FxFee row) {
    return new com.outpost.fx.FxFee(
        row.fxFeeId(),
