...
final class CountryStaticDataRepository
  ...
  private final CountryStaticDataMapper mapper;

  /** Creates a typed repository. */
  CountryStaticDataRepository(CountryStaticDataMapper mapper) {
    this.mapper = mapper;
  }
  ...
  public CountryRecord toDatabaseRecord(Country value) {
    return new CountryRecord(value.getCountryId(), value.getIsoCode(), value.getName());
  }
  ...
  public Country toDomainValue(CountryRecord record) {
    return Countries.fromIsoCode(record.isoCode())
        .filter(
            value ->
                value.getCountryId() == record.countryId() && value.getName().equals(record.name()))
        .orElseThrow(
            () ->
