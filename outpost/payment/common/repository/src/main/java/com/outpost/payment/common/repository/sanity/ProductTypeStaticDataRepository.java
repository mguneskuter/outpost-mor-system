...
final class ProductTypeStaticDataRepository
  ...
  private final ProductTypeStaticDataMapper mapper;

  /** Creates a typed repository. */
  ProductTypeStaticDataRepository(ProductTypeStaticDataMapper mapper) {
    this.mapper = mapper;
  }
  ...
  public ProductTypeRecord toDatabaseRecord(ProductType value) {
    return new ProductTypeRecord(value.getProductTypeId(), value.getCode());
  }

  @Override
  public ProductType toDomainValue(ProductTypeRecord record) {
    return ProductTypes.fromCode(record.code())
        .filter(value -> value.getProductTypeId() == record.productTypeId())
        .orElseThrow(
            () ->
