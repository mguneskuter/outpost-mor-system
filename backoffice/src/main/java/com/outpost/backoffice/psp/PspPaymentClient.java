...
public final class PspPaymentClient {
  ...
  private final @Nullable URI pspBaseUrl;
  private final HttpClient http;
  private final ObjectMapper json;

  /**
   * Creates a client that presents the simulator's API key on every payment and, when {@code
   * pspBaseUrl} is given, pays at that base URL under each payment link's path instead of at the
   * link as published.
   */
  public PspPaymentClient(
      String pspApiKey, @Nullable URI pspBaseUrl, HttpClient http, ObjectMapper json) {
  ...
  public void pay(String paymentLink, String pspReference, String cardNumber) {
    HttpRequest request =
        HttpRequest.newBuilder(paymentUri(paymentLink))
  ...
  private URI paymentUri(String paymentLink) {
    URI link = URI.create(paymentLink);
    return pspBaseUrl == null ? link : pspBaseUrl.resolve(link.getRawPath());
  }
