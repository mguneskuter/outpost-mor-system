...
"${repository_root}/outpost/gradlew" -p outpost \
    :framework:persistence:writeCliClasspath :framework:persistence:compileCliJava >/dev/null

java -Doutpost.migration.location="${repository_root}/outpost/db/migration" \
    -cp "$(cat "${repository_root}/outpost/framework/persistence/build/cli/classpath.txt")" \
    com.outpost.framework.persistence.flyway.FlywayCli \
